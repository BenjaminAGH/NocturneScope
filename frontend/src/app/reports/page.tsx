        ...
        try {
            ...
            const finalData = {
                ...
                stats: scope === 'group'
                    ? [
                        ...
                        ...(selectedMetrics.includes('cpu') ? [{ name: t('cpuUsage') || 'Avg CPU (%)', min: 0, max: 0, avg: groupAvgCpu }] : []),
                        ...(selectedMetrics.includes('ram') ? [{ name: t('ramUsage') || 'Avg RAM (%)', min: 0, max: 0, avg: groupAvgRam }] : []),
                        ...(selectedMetrics.includes('temp') ? [{ name: t('temp') || 'Avg Temp (°C)', min: 0, max: 0, avg: groupAvgTemp }] : []),
                    ]
                    : results[0].stats,
            ...
            setReportData({
                ...
                labels: {
                    ...
                    generatedBy: t('footerDescription') || "Generated by NocturneScope",
                    cpuUsage: t('cpuUsage'),
                    ramUsage: t('ramUsage'),
                    netRx: t('netRx'),
                    netTx: t('netTx'),
                    groupTrends: t('groupTrends'),
